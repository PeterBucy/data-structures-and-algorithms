...
const listFoods = (recipe) => {
  ...
  for (let i in recipe.ingredients){
    ...
    let space1 = recipe.ingredients[i].indexOf(' ');

    let space2 = recipe.ingredients[i].indexOf(' ', space1 + 1);

    result.push(recipe.ingredients[i].slice(space2 + 1));
...
const stepActions = (recipe) => {
  ...
  for(let i in recipe.steps){
    ...
    let actions = recipe.steps[i].split(' ');

    result.push(actions.shift());

...
const notInFirstArray = (forbiddenValues, arr) => {
    return arr.filter(nums => {
        return !forbiddenValues.includes(nums);
      });
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
    return arr.filter( stats => (minBaseStat < stats.baseStat));
};
...
const getStatName = (arr, minBaseStat) => {
  let pokimon = arr.filter( stats => (minBaseStat < stats.baseStat));
    let yugio = pokimon.map( stats => stats.stat.name);
    return yugio};

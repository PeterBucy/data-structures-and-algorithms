...
const getHouses = (arr) => {
  ...
  for( let i in arr) {
      ...
      houseArr.push(Object.values(arr[i])[3]);
...
const hasChildrenValues = (arr, character) => {
  ...
  for ( let i in arr) {
    ...
    if ( arr[i].name === character) {
      ...
      let hasChildren = Object.values(arr[i]);

        if (hasChildren[2].length < 1) {
...
const hasChildrenEntries = (arr, character) => {
  ...
  for ( let i in arr) {
    ...
    if ( arr[i].name === character) {
      ...
      let hasChildren = Object.entries(arr[i])[2];

        if (hasChildren[1].length === 0) {
